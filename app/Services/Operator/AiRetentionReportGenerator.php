...
class AiRetentionReportGenerator
{
     ...
     */
    public function generate(array $operatorIds, array $channelIds, ?int $limit = null, bool $save = false, ?int $testSaveUserId = null): array
    {
        $t0 = microtime(true);
        ...
        $messages = $this->getMessages($operatorIds, $channelIds);
        ...
        // Ограничители для тестового режима, чтобы не улетать в таймаут
        $attemptCap = null;
        $deadlineAt = null;
        if ($save === false) {
            $attemptCap = $limit !== null ? max($limit, (int) ceil($limit * 1.5)) : 5;
            $deadlineAt = microtime(true) + 55; // мягкий бюджет по времени, ~55с
            DbLog::info("[AI-Retention] generate: test-mode caps: attemptCap={$attemptCap}, deadlineSec=55");
        }
        ...
        foreach ($dialogs as $key => $dialog) {
            ...
            if ($attemptCap !== null && $attempted >= $attemptCap) {
                DbLog::info("[AI-Retention] generate: stopped by attemptCap (attempted={$attempted})");
                break;
            }
            if ($deadlineAt !== null && microtime(true) >= $deadlineAt) {
                DbLog::info("[AI-Retention] generate: stopped by deadline (~55s)");
                break;
            }
            $processed++;
            $attempted++;
            ...
            // В тестовом режиме сохраняем в test-таблицу только первые $limit валидных записей
     ...
     */
    private function getMessages(array $operatorIds, array $channelIds): Collection
    {
    ...
    private function groupMessages(Collection $messages, bool $testMode = false, string $prefer = 'operator'): array
    {
        ...
        // Оставляем только диалоги с 3 и более сообщений
        return array_filter($dialogs, fn(array $dialog) => count($dialog['messages']) >= 3);
    }
    ...
    private function processDialog(array $dialog, string $template, Client $openai, bool $save, ?int $testSaveUserId = null): AiRetentionReportListDto|AiRetentionReportDto|null
    {
        try {
            ...
            // save=false → тестовый режим: при наличии userId сохраняем в тестовую таблицу
            if ($testSaveUserId !== null) {
                $saveStart = microtime(true);
                AiRetentionReportTest::query()->create([
